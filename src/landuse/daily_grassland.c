                    ...
                    )                             /** \return runoff (mm/day) */
{
  ...
  if(stand->cell->lateral_water>0 && stand->soil.iswetland==TRUE && stand->frac>0.001)
  {
    ...
    runoff+=infil_perc(stand,rainmelt+rw_apply+irrig_apply+lateral_in, vol_water_enth,climate->prec+rw_apply+irrig_apply,irrig_apply,&return_flow_b,npft,ncft,config);     //enthalpy of lateral influx?? should be the same T as in the local stand
  ...
  else
  {
    ...
    if(rainmelt+rw_apply+irrig_apply < 0)
      fprintf(stderr,"WARNING044: Negative water input to infiltration on day %d of year %d in cell (%s): rainmelt=%g, rw_apply=%g, irrig_apply=%g\n",
              day,year,sprintcoord(line,&stand->cell->coord),rainmelt, rw_apply, irrig_apply);
#endif
    runoff+=infil_perc(stand,rainmelt+rw_apply+irrig_apply, vol_water_enth,climate->prec+rw_apply+irrig_apply,irrig_apply,&return_flow_b,npft,ncft,config);
  }
    ...
    foreachpft(pft,p,&stand->pftlist)
    {
      ...
      else
      {
        ...
        if(getlandusetype(pft->stand)==GRASSLAND)
          getoutput(output,LITFALLC_MGRASS,config)+=grass->ind.leaf.carbon*grasspar->turnover.leaf/NDAYYEAR*pft->nind*stand->frac;
        if(isnatural(pft->stand))
          getoutput(output,LITFALLC_NV,config)+=grass->ind.leaf.carbon*grasspar->turnover.leaf/NDAYYEAR*pft->nind*stand->frac;
        ...
        if(getlandusetype(pft->stand)==GRASSLAND)
          getoutput(output,LITFALLN_MGRASS,config)+=grass->ind.leaf.nitrogen*grasspar->turnover.leaf/NDAYYEAR*pft->nind*pft->par->fn_turnover*stand->frac;
        if(isnatural(pft->stand))
          getoutput(output,LITFALLN_NV,config)+=grass->ind.leaf.nitrogen*grasspar->turnover.leaf/NDAYYEAR*pft->nind*pft->par->fn_turnover*stand->frac;
        ...
        if(getlandusetype(pft->stand)==GRASSLAND)
          getoutput(output,LITFALLC_MGRASS,config)+=grass->ind.root.carbon*grasspar->turnover.root/NDAYYEAR*pft->nind*stand->frac;
        if(isnatural(pft->stand))
          getoutput(output,LITFALLC_NV,config)+=grass->ind.root.carbon*grasspar->turnover.root/NDAYYEAR*pft->nind*stand->frac;
        ...
        if(getlandusetype(pft->stand)==GRASSLAND)
          getoutput(output,LITFALLN_MGRASS,config)+=grass->ind.root.nitrogen*grasspar->turnover.root/NDAYYEAR*pft->nind*pft->par->fn_turnover*stand->frac;
        if(isnatural(pft->stand))
          getoutput(output,LITFALLN_NV,config)+=grass->ind.root.nitrogen*grasspar->turnover.root/NDAYYEAR*pft->nind*pft->par->fn_turnover*stand->frac;
