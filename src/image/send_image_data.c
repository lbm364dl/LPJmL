        ...
        if (stand->type->landusetype==WOODPLANTATION)
        {
          foreachpft(pft,p,&stand->pftlist)
            switch (pft->par->type)
            {
              ...
              case GRASS:
                grass = pft->data;
                biomass_image_wp[cell].leaves += (float)grass->ind.leaf.carbon*(float)stand->frac*(float)pft->nind;
                biomass_image_wp[cell].roots += (float)grass->ind.root.carbon*(float)stand->frac*(float)pft->nind;
                break;
            } /* of switch */
